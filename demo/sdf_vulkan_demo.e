note
	description: "[
		SDF Vulkan Demo - GPU-accelerated ray marching using Vulkan compute shaders.

		Demonstrates simple_vulkan integration with simple_sdf for high-performance
		rendering at 60+ FPS at 4K resolution on modern GPUs.
...
feature {NONE} -- Initialization
	...
	make
			-- Run the Vulkan SDF demo.
		do
			print ("SDF Vulkan Demo%N")
			print ("==============%N%N")
	...
	initialize_vulkan
		...
		do
			...
			if l_ctx.is_valid then
				...
				shader_path := shader_directory + "sdf_buffer_output.spv"
				...
				if l_shader.is_valid then
					...
					print ("Run: glslc shaders/sdf_buffer_output.comp -o " + shader_path + "%N")
				end
			end
		end
	...
	initialize_display
		...
		do
			...
			if attached ctx as l_ctx then
				l_title := "SDF Vulkan Demo - " + l_ctx.device_name
			else
				l_title := "SDF Vulkan Demo"
			end
...
feature {NONE} -- Demo Loop
			...
			if attached window as w and attached display_buffer as b and
			   ...
			   attached output_buffer as ob and attached params_buffer as pb then
				...
				loop
					...
					copy_to_display (pixels, l_buffer)
	...
	copy_to_display (a_pixels: MANAGED_POINTER; a_buffer: MINIFB_BUFFER)
			-- Copy GPU output to display buffer (bulk memcpy).
		do
			a_buffer.copy_from (a_pixels.item, width * height * 4)
		end
